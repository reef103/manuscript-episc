...
knitr::opts_chunk$set(fig.align = 'center', fig.dim = c(6, 4), echo=FALSE, cache=FALSE)
```
...
```{r PCA-trajecories, out.width='50%', fig.show='hold', fig.cap="Temporal trajectories for the first 3 principal components of the protein activity signatures for each of the treatments, and 2-dimensional projection of the protein activity signatures based on PCA."}
...
expression profile by PLATE-Seq technology (Table \@ref(tab:selected-prteins-table)).
...
```{r strong-nodes}
n_strong_nodes <- length(V(induced_subgraph(ntw, names(stbFET[stbFET>2])))$name)
```
