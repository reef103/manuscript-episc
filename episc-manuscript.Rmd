...
exportGESsignature(pc2_sig, n = 100, file = file.path(working_dir, "PC2-genes.tsv"), gz = FALSE)
...
go_tbl <- GOtable(go_pc1, thr = 0.01, adjust = "fdr", pc1_sig)
write.table(go_tbl, file = file.path(working_dir, "GO-pilotPC1.tsv"), quote = FALSE,
  sep = "\t", row.names = FALSE, col.names = TRUE)
knitr::kable(go_tbl, row.names = FALSE, colnames = TRUE,
  align = "llrrcc", caption = "GO-BP terms significantly enriched in the PC1 signature (FDR < 0.01).")
```
...
# To avoid potential issues/differences in the community results based on
# different versions of the igraph package and associated algorithms we load
# the pre-computed communities below
com_membership <- readCommunityFromFile(data_dir)
...
p2 <- violinCommunitiesTest(log2(degree_tbl[, "Betweenness"][pos]+1), com_membership[pos],
                      community_colors, ylab="log2(Betweenness+1)", xlab="Community")
